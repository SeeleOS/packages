...
use crate::package::xorg::{LibX11, LibXfixes, XorgProto};
...
pub struct LibXdamage;

impl Package for LibXdamage {
    fn name(&self) -> &'static str { "libxdamage" }
    fn dependencies(&self) -> Vec<Box<dyn Package>> {
        vec![Box::new(XorgProto), Box::new(LibX11), Box::new(LibXfixes)]
    }
...
impl TarballFetch for LibXdamage {
    fn tarball_url(&self) -> Vec<&'static str> {
        vec!["https://www.x.org/archive/individual/lib/libXdamage-1.1.7.tar.gz"]
    }
}
