...
use crate::package::xorg::{LibX11, LibXext, XorgProto, XorgUtilMacros};
...
pub struct LibXinerama;

impl Package for LibXinerama {
    fn name(&self) -> &'static str { "libxinerama" }
    fn dependencies(&self) -> Vec<Box<dyn Package>> {
        vec![Box::new(LibX11), Box::new(LibXext), Box::new(XorgUtilMacros), Box::new(XorgProto)]
    }
...
impl TarballFetch for LibXinerama {
    fn tarball_url(&self) -> Vec<&'static str> {
        vec!["https://www.x.org/archive/individual/lib/libXinerama-1.1.6.tar.gz"]
    }
}
