...
use crate::package::xorg::{LibXau, LibXdmcp, XcbProto, XorgProto};
...
impl Package for LibXcb {
    fn name(&self) -> &'static str { "libxcb" }
    fn dependencies(&self) -> Vec<Box<dyn Package>> {
        vec![Box::new(XorgProto), Box::new(LibXau), Box::new(LibXdmcp), Box::new(XcbProto)]
    }
...
impl TarballFetch for LibXcb {
    fn tarball_url(&self) -> Vec<&'static str> {
        vec!["https://www.x.org/archive/individual/lib/libxcb-1.17.0.tar.xz"]
    }
}
