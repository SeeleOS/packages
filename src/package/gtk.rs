...
make_meson_packages!(
    ...
    { Gtk4, "gtk4", tarball_url = "https://download.gnome.org/sources/gtk/4.18/gtk-4.18.6.tar.xz", dependencies = [Cairo, GdkPixbuf, Glib2, Pango, LibEpoxy, LibX11, LibXcursor, LibXi, LibXrandr, LibXrender], configure = { args = vec!["-Dintrospection=disabled".to_string(), "-Dbuild-demos=false".to_string(), "-Dbuild-examples=false".to_string(), "-Dbuild-tests=false".to_string(), "-Dwayland-backend=false".to_string(), "-Dx11-backend=true".to_string(), "-Dbroadway-backend=false".to_string(), "-Dmedia-gstreamer=disabled".to_string(), "-Dprint-cups=disabled".to_string()] } },
);
